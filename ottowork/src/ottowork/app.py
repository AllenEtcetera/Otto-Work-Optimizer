    ...
    if notif_on:
        def notify():
            while notif_on:
                ...
                if notif_on:
                    playsound('resources/hourpass.mp3')
                    notification.notify(
                        title='Otto Hour',
...
def get_resource_path(relative_path):
    if hasattr(sys, '_MEIPASS'):
        # Running as a bundled application (PyInstaller)
        base_path = sys._MEIPASS
    elif 'BRIEFCASE_BUILD_DIR' in os.environ:
        # Running in Briefcase development mode
        base_path = os.environ['BRIEFCASE_BUILD_DIR']
...
def create_window():
    ...
    # Otto Input button
    input_button = tk.Button(rootHub, text="Input", command=lambda:subprocess.Popen(['python', get_resource_path('resources/OttoInput.py')]))
    input_button.grid(row=0,column=0,pady=5,padx=5)
    # Otto List button
    input_button = tk.Button(rootHub, text="List", command=lambda:subprocess.Popen(['python', get_resource_path('resources/OttoList.py')]))
    input_button.grid(row=0,column=1,pady=5,padx=5)
    # Otto Math button
    input_button = tk.Button(rootHub, text="Math", command=lambda:subprocess.Popen(['python', get_resource_path('resources/OttoMath.py')]))
    input_button.grid(row=0,column=2,pady=5,padx=5)
    # Otto Note button
    input_button = tk.Button(rootHub, text="Notes", command=lambda:subprocess.Popen(['python', get_resource_path('resources/OttoNote.py')]))
    input_button.grid(row=1,column=0,pady=5,padx=5)
    # Otto Overwrite button
    input_button = tk.Button(rootHub, text="Overwrite", command=lambda:subprocess.Popen(['python', get_resource_path('resources/OttoOverwrite.py')]))
    input_button.grid(row=1,column=1,pady=5,padx=5)
    # Otto Remind button
    input_button = tk.Button(rootHub, text="Remind", command=lambda:subprocess.Popen(['python', get_resource_path('resources/OttoRemind.py')]))
    input_button.grid(row=1,column=2,pady=5,padx=5)
    # Otto Sort button
    input_button = tk.Button(rootHub, text="Sort", command=lambda:subprocess.Popen(['python', get_resource_path('resources/OttoSort.py')]))
